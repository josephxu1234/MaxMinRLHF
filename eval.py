...
def main(args):
    if args.checkpoint_dirs!=None:
        ...
        # Step 1: Initialize empty model to infer map without loading weights
        with init_empty_weights():
            model_init = AutoModelForCausalLM.from_config(config)

        # Step 2: Define available memory and infer device map
        device_map = infer_auto_device_map(
            model_init,
            max_memory={0: "8GiB", "cpu": "64GiB"},  # Leave headroom
            no_split_module_classes=["LlamaDecoderLayer"]  # Change if needed
        )

        # Step 3: Now load the full model
