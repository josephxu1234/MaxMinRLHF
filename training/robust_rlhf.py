...
class ScriptArguments:
    ...
    reward_model_name1: Optional[str] = field(default="", metadata={"help": "the reward model name"})
    reward_model_name2: Optional[str] = field(default="", metadata={"help": "the reward model name"})
...
reward_model_name1 = script_args.reward_model_name1
reward_model_name2 = script_args.reward_model_name2
config = PPOConfig(
...
if script_args.val_dataset_name:
    val_dataset = build_dataset(tokenizer, script_args.val_dataset_name)
...
reward_model1 = AutoModelForSequenceClassification.from_pretrained(
...
reward_model1 = get_peft_model(reward_model1, rm_peft_config)
ckpt_params = torch.load(reward_model_name1, map_location=f'cuda:{current_device}')
loading_cnt = 0
for name, param in reward_model1.named_parameters():
...
reward_model1.to(f'cuda:{current_device}')
freeze_trainable_parameters(reward_model1)


reward_model2 = AutoModelForSequenceClassification.from_pretrained(
    config.model_name, num_labels=1, torch_dtype=torch.bfloat16,
    load_in_4bit=True
)
rm_peft_config = LoraConfig(
...
reward_model2 = get_peft_model(reward_model2, rm_peft_config)
ckpt_params = torch.load(reward_model_name2, map_location=f'cuda:{current_device}')
loading_cnt = 0
for name, param in reward_model2.named_parameters():
    temp_name = name.replace('.default', '')
    temp_name = temp_name.replace('.original_module', '')
    if temp_name in ckpt_params:
        param.data = ckpt_params[temp_name]
...
print(f'loaded {loading_cnt} LoRA weights for the reward model.')
reward_model2.to(f'cuda:{current_device}')
freeze_trainable_parameters(reward_model2)
...
for steps, batch in tqdm(enumerate(ppo_trainer.dataloader)):
    ...
    reward_outputs1 = reward_model1(input_ids = input_ids.to(reward_model1.device))[0]
    reward_outputs2 = reward_model2(input_ids = input_ids.to(reward_model2.device))[0]
    #reward_outputs = nn.functional.logsigmoid(reward_outputs)
    rewards1 = [torch.tensor(output[0].float()) - script_args.reward_baseline for output in reward_outputs1]
    rewards2 = [torch.tensor(output[0].float()) - script_args.reward_baseline for output in reward_outputs2]
    rewards = [min(rewards1[i], rewards2[i]) for i in range(len(rewards1))]
    v_min, v_max, v_mean = get_reward_stats(rewards)
    # v_min2, v_max2, v_mean2 = get_reward_stats(rewards2)
    # # compare the two rewards
    # if v_mean1 > v_mean2:
    #     rewards = rewards1
    #     v_min, v_max, v_mean = v_min1, v_max1, v_mean1
    # else:
    #     rewards = rewards2
    #     v_min, v_max, v_mean = v_min2, v_max2, v_mean2
    # Run PPO step
            ...
            with torch.no_grad():
                ...
                for i, val_batch in tqdm(enumerate(val_dataloader)):
                    ...
                    if len(question_tensors) != 0:
                        ...
                        # Compute sentiment score
                        texts = [q + r for q, r in zip(val_batch["original_query"], val_batch["response"])]
                        input_ids = tokenizer(texts, max_length=script_args.max_length ,return_tensors="pt", padding=True, truncation=True).input_ids
                        reward_outputs1 = reward_model1(input_ids = input_ids.to(reward_model1.device))[0]
                        rewards1 = [torch.tensor(output[0]).cpu().float() - script_args.reward_baseline for output in reward_outputs1]
                        reward_outputs2 = reward_model2(input_ids = input_ids.to(reward_model2.device))[0]
                        rewards2 = [torch.tensor(output[0]).cpu().float() - script_args.reward_baseline for output in reward_outputs2]
                        # get the minimum between two rewards
                        rewards = [min(rewards1[i], rewards2[i]) for i in range(len(rewards1))]
